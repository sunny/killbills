...
ActiveAdmin::Dashboards.build do
  ...
  section "Recent Bills" do
    ul do
      Bill.last(5).reverse.collect do |bill|
        li link_to("#{bill.user.display_name}: #{bill.title}", admin_bill_path(bill)) + " by " + link_to(bill.user.display_name, admin_user_path(bill.user))
      end
    end
  end
  ...
  section "Recent Users" do
    ul do
      User.last(5).reverse.collect do |user|
